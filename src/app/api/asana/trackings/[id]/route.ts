// src/app/api/asana/trackings/route.ts - Updated para projeto "Operacional"
...
export async function GET() {
  try {
    ...
    // 4. Filter tracking tasks (tasks that look like shipments)
    const trackingTasks = tasks.filter(task => {
      if (!task.name) return false;
      
      // Filter for tasks that contain tracking-like patterns
      const name = task.name.toLowerCase();
      return (
        name.includes('porcelanosa') ||
        name.includes('univar') ||
        name.includes('°') ||
        name.match(/\d+.*[a-z]/) || // Pattern like "15porcelanosa" or "645univar"
        (task.custom_fields && task.custom_fields.length > 0) // Has custom fields
      );
    });

    console.log(`Filtered to ${trackingTasks.length} tracking tasks`);

    // 5. Transform tasks to tracking format
    const trackings = trackingTasks.map(task => transformAsanaTaskToTracking(task));
